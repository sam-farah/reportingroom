...
**Report scoping (added 2026-08-03):** all /api/reports* routes now authorise via `resolveAuthorisedReport` (per-id) or `getClinicReports`/`getRecentClinicReports` (lists). Legacy reports have `clinicId NULL` (~124 in prod) — visibility falls back to the linked patient's clinic; a null-clinic + null-patient report is invisible by design. Any NEW report route must use these helpers, not `getReport`/`getAllReports` directly. Unlinked worksheets (no patientId) are still globally claimable in /api/reports/generate — known gap, single-clinic acceptable.
