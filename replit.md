...
## Pending Features
- **Assessment of Benefits (AoB) — Medicare claim confirmation + patient signature (COMPLETE — placeholder document layout pending the real Medicare form upload)**: Two decoupled steps.
  - *Step 1, blocking, at study completion*: When a sonographer clicks "Sono Complete" / "Mark Sono Complete" in Reporting Room, a confirmation dialog (prefilled via `calculateVisitBilling` from the report's `studyType`) shows the suggested Medicare item numbers, descriptions, and fees, editable line-by-line (add/remove items, edit fee) before the study can be marked complete — `POST /api/reports/:id/sonographer-complete` now requires a non-empty `items` array and 400s otherwise. On success it snapshots patient/physician/referrer details plus the confirmed items into a new `assessment_of_benefit_forms` row (status `pending_signature`) and auto-completes the matching same-day appointment if one is found.
  - *Step 2, later, decoupled*: The patient signature is captured separately — often by admin staff, not the sonographer — from the calendar appointment detail dialog. A "Complete Assessment of Benefits form" button appears there whenever a `pending_signature` AoB form exists for that appointment (`GET /api/appointments/:id/assessment-of-benefit`); it opens a signature-pad dialog (same canvas/pointer-event pattern as the kiosk consent capture) showing the confirmed items/total, and submitting calls `POST /api/assessment-of-benefit/:id/sign` with the signature PNG data URL, which generates the signed document and flips the form to `status: "signed"`.
  - Data: `assessment_of_benefit_forms` table (clinic/appointment/report/patient-scoped snapshot); `physicians.providerNumber` and `appointments.referringDoctorProviderNumber` were added to support the physician/referrer provider numbers shown on the form. Storage CRUD in `server/storage.ts`; routes and document generation in `server/routes.ts`.
  - Not yet done: the actual visual layout/wording of the generated AoB document is a placeholder — it will be redone once the user supplies the real Medicare form to match.
