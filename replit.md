...
- **Assessment of Benefits (AoB) — Medicare claim confirmation + patient signature (COMPLETE)**: Two decoupled steps.
  ...
  - *Step 2, later, decoupled*: The patient signature is captured separately — often by admin staff, not the sonographer — from the calendar appointment detail dialog. A "Complete Assessment of Benefits form" button appears there whenever a `pending_signature` AoB form exists for that appointment (`GET /api/appointments/:id/assessment-of-benefit`); it opens a signature-pad dialog (same canvas/pointer-event pattern as the kiosk consent capture) showing the confirmed items/total, and submitting calls `POST /api/assessment-of-benefit/:id/sign` with the signature PNG data URL, which generates the signed documents and flips the form to `status: "signed"`.
  - Document layout follows the real Medicare Bulk Bill Webclaim assessment form's field groups/labels (not a pixel replica): Patient Details, Referral Details (incl. LSPN if the clinic has one set, "In-hospital referral: No" and "Number of patients attended: 1" as fixed defaults since they're always true for this workflow), Practitioner Who Rendered the Service (deliberately reuses the referring doctor's name/provider number per clinic instruction — the app has no separate "rendering practitioner" concept), Services table, and an Assignment of Benefit declaration + signature block. Equipment Number and SCP are intentionally omitted (not tracked by the app). Two copies are generated per signature — Practitioner/clinic copy and Patient copy (patient copy has a short retain-this-copy footer) — both saved to `server/routes.ts`'s `generateAssessmentOfBenefitDocument()` and attached to the patient file as separate documents.
  - Data: `assessment_of_benefit_forms` table (clinic/appointment/report/patient-scoped snapshot, incl. `patientDateOfBirth`, `referringDoctorAddress`, `referralDate`, `patientDocumentUrl`); `physicians.providerNumber`, `appointments.referringDoctorProviderNumber`, and `clinics.locationSpecificPracticeNumber` (editable in Admin → Clinic Settings) support the fields shown on the form. Storage CRUD in `server/storage.ts`; routes and document generation in `server/routes.ts`.
